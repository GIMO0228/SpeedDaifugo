...
function renderJokerSettingItem(type) {
    ...
    // ボタン分離案：メイン領域（切り替え）と詳細ボタン（i）
    itemEl.innerHTML = `
        <div class="joker-setting-main">
            <div class="joker-setting-icon">${joker.getIcon()}</div>
            <div class="joker-setting-name">${joker.getName()}</div>
            <div class="joker-setting-status">${isEnabled ? '✅' : 'OFF'}</div>
        </div>
        <button class="joker-info-btn" aria-label="詳細">ℹ️</button>
    `;

    // ===== イベントリスナー設定 =====
    const mainArea = itemEl.querySelector('.joker-setting-main');
    const infoBtn = itemEl.querySelector('.joker-info-btn');

    // メイン領域クリック：有効/無効切り替え
    mainArea.addEventListener('click', () => {
        const newEnabled = gameSystem.toggleJokerDisabled(type);
        const statusEl = itemEl.querySelector('.joker-setting-status');
